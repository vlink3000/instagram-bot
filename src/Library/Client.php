...
use App\Library\Exception\ServerMessageThrower;
use App\Library\Middleware\FakeCookies;
use App\Library\Middleware\ZeroRating;
...
class Client
{
    ...
    /**
     * @var \App\Library\Middleware\FakeCookies
     */
    private $_fakeCookies;

    /**
     * @var \App\Library\Middleware\ZeroRating
     */
    private $_zeroRating;
     ...
     * Constructor.
     *
     * @param \App\Library\Instagram $parent
     */
        ...
        HttpResponseInterface $httpResponse)
    {
        ...
        if (!is_array($jsonArray)) {
            ...
            switch ($httpStatusCode) {
                case 400:
                    throw new \App\Library\Exception\BadRequestException('Invalid request options.');
                case 404:
                    throw new \App\Library\Exception\NotFoundException('Requested resource does not exist.');
                default:
                    throw new \App\Library\Exception\EmptyResponseException('No response from server. Either a connection or configuration error.');
            }
        }
        ...
        if (!$responseObject->isOk()) {
            if ($responseObject instanceof \App\Library\Response\DirectSendItemResponse && $responseObject->getPayload() !== null) {
     ...
     *
     * @throws \App\Library\Exception\NetworkException                For any network/socket related errors.
     * @throws \App\Library\Exception\ThrottledException              When we're throttled by server.
     * @throws \App\Library\Exception\RequestHeadersTooLargeException When request is too large.
     *
        ...
        array $guzzleOptions = [])
    {
        ...
        try {
            ...
            throw new \App\Library\Exception\NetworkException($e);
        }
        ...
        case 429: // "429 Too Many Requests"
            throw new \App\Library\Exception\ThrottledException('Throttled by Instagram because of too many API requests.');
            break;
        case 431: // "431 Request Header Fields Too Large"
            throw new \App\Library\Exception\RequestHeadersTooLargeException('The request start-line and/or headers are too large to process.');
            break;
